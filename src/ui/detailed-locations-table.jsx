...
class DetailedLocationsTable extends React.PureComponent {
  ...
  requirementsTooltip(generalLocation, detailedLocation) {
    ...
    return <RequirementsTooltip requirements={requirements} />;
  }

  itemTooltip(generalLocation, detailedLocation) {
    const { trackerState } = this.props;

    const itemForLocation = trackerState.getItemForLocation(
      generalLocation,
      detailedLocation,
    );
  ...
  detailedLocation(locationInfo, numColumns, numRows) {
    ...
    const isHinted =
      hintedLocations && hintedLocations[openedLocation]?.has(location);
    const hintedClass =
      isHinted && color !== "checked-location" ? " hinted-location" : "";

    const locationElement = (
      <div
        className={`detail-span ${color} ${fontSizeClassName}${hintedClass}`}
    ...
    if (disableLogic || isLocationChecked) {
      ...
      locationContent = (
        <Tooltip tooltipContent={itemTooltip}>{locationElement}</Tooltip>
      );
    } else {
  ...
  render() {
    ...
    if (
      ...
      const isDisabled =
        isBossRequired && !LogicHelper.anyNonRequiredBossesRemaining();
