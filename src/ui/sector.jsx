...
class Sector extends React.PureComponent {
  ...
  chestsCounter() {
    ...
    let isHinted = false;
    if (hintedLocations && hintedLocations[island]) {
      for (const loc of hintedLocations[island]) {
        if (!trackerState.isLocationChecked(island, loc)) {
          isHinted = true;
          break;
        }
      }
    }
    const className = `chests-counter ${color}${isHinted ? " hinted-sector" : ""}`;
  ...
  chartItem() {
    ...
    if (
      ...
      !LogicHelper.islandHasProgressItemChart(island)
    ) {
      return null;
    }
  ...
  chartItemVanilla() {
    ...
    const { chartName, chartType } = LogicHelper.vanillaChartForIsland(island);
  ...
  entranceExitItems() {
    const { disableLogic, island, logic, viewingEntrances } = this.props;

    if (viewingEntrances) {
      const islandEntrances = logic.entrancesListForIsland(island, {
        disableLogic,
      });
      return _.map(islandEntrances, (islandEntrance) =>
        this.islandEntrance(islandEntrance),
      );
    }
  ...
  render() {
    ...
    const setSelectedLocationFunc = () =>
      setSelectedLocation({ locationName: island });
