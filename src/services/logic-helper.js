...
class LogicHelper {
  ...
  static MAIN_DUNGEONS = _.filter(DUNGEONS, (dungeon) =>
    this.isMainDungeon(dungeon),
  );
  ...
  static ALL_ITEMS = _.concat(
    _.map(ISLAND_ENTRANCES, (entranceData) =>
      this.entryName(entranceData.internalName),
    ),
    CHARTS,
    _.map(ISLANDS, (island) => this.randomizedChartForIsland(island)),
    _.map(DUNGEON_ENTRANCES, (entranceData) =>
      this.entryName(entranceData.internalName),
    ),
    _.keys(ITEMS),
    _.keys(KEYS),
  ...
  static ALL_TREASURE_CHARTS = _.range(
    1,
    CHARTS.length - this.NUM_TRIFORCE_CHARTS + 1,
  ).map((number) => `Treasure Chart ${number}`);

  static ALL_TRIFORCE_CHARTS = _.range(1, this.NUM_TRIFORCE_CHARTS + 1).map(
    (number) => `Triforce Chart ${number}`,
  );

  static CAN_ACCESS_ITEM_LOCATION_REGEX = /Can Access Item Location "([^"]+)"/;

  static HAS_ACCESSED_OTHER_LOCATION_REGEX =
  ...
  static entrancesForIsland(islandName) {
    return _.compact(
      _.map(
        _.concat(this.#filterDungeonEntrances(), this.#filterIslandEntrances()),
        (entranceData) =>
  ...
  static exitsForIsland(islandName) {
    return _.compact(
      _.map(this.#filterIslandEntrances(), (entranceData) =>
  ...
  static entrancesForDungeon(zoneName) {
    return _.compact(
      _.map(this.#filterDungeonEntrances(), (entranceData) =>
  ...
  static allRandomEntrances() {
    return _.concat(this.#allDungeonEntrances(), this.#allIslandEntrances());
  }
  ...
  static randomExitsForEntrance(entranceName) {
    ...
    const parentExit = _.findKey(NESTED_ENTRANCES, (nestedEntrances) =>
      _.includes(nestedEntrances, entranceName),
    );
  ...
  static isRandomizedChart(item) {
    return (
      ...
      /(Treasure|Triforce) Chart (\d)+/.test(item)
    );
  }
  ...
  static bossLocation(dungeonName) {
    ...
    const detailedLocations =
      Locations.detailedLocationsForGeneralLocation(dungeonName);
    ...
    return requirementsForLocation.evaluate({
      isItemTrue: (requirement) => {
        const itemCountRequirement =
          this.parseItemCountRequirement(requirement);
  ...
  static requirementsForLocation(
    generalLocation,
    detailedLocation,
    isFlattened,
  ) {
  ...
  static requirementsForEntrance(entranceName) {
    ...
    const rawRequirements = this.#booleanExpressionForRequirements(
      macroName,
      false,
    );
    return this.#simplifiedItemRequirements(rawRequirements);
  ...
  static prettyNameForItemRequirement(itemRequirement) {
    ...
    if (!_.isNil(itemCountRequirement)) {
      ...
      return (
        this.#prettyNameOverride(itemName, countRequired) || itemRequirement
      );
    }
  ...
  static islandHasProgressItemChart(islandName) {
    ...
    if (chartType === this.CHART_TYPES.TREASURE) {
      return Settings.getOptionValue(
  ...
  static bannedLocationsForZone(zoneName, { includeAdditionalLocations }) {
    const detailedLocations =
      Locations.detailedLocationsForGeneralLocation(zoneName);
    ...
    const additionalBannedLocations = _.get(
      ADDITONAL_BANNED_LOCATIONS,
      zoneName,
      [],
    );

    return _.concat(zoneLocations, additionalBannedLocations);
  }
  ...
  static setBossRequired(dungeonName) {
    this.nonRequiredBossDungeons = _.without(
      this.nonRequiredBossDungeons,
      dungeonName,
    );
  ...
  static setBossNotRequired(dungeonName) {
    this.nonRequiredBossDungeons = _.concat(
      this.nonRequiredBossDungeons,
      dungeonName,
    );
    this.#invalidateForNonRequiredBosses();
  ...
  static rawRequirementsForLocation(
    generalLocation,
    detailedLocation,
    isFlattened,
  ) {
  ...
  static #checkOtherLocationRequirement(requirement, isFlattened) {
    let otherLocationMatch = _.get(
      requirement.match(this.CAN_ACCESS_ITEM_LOCATION_REGEX),
      1,
    );

    if (_.isNil(otherLocationMatch) && isFlattened) {
      otherLocationMatch = _.get(
        requirement.match(this.HAS_ACCESSED_OTHER_LOCATION_REGEX),
        1,
      );
    }
    ...
    if (otherLocationMatch) {
      ...
      return this.rawRequirementsForLocation(
        generalLocation,
        detailedLocation,
        isFlattened,
      );
  ...
  static #checkPredeterminedItemRequirement(requirement) {
    ...
    const itemCountRequirement =
      LogicHelper.parseItemCountRequirement(requirement);
    if (!_.isNil(itemCountRequirement)) {
  ...
  static #checkBossRequirement(requirement, isFlattened) {
    ...
    if (_.isNil(this.nonRequiredBossDungeons)) {
      // istanbul ignore next
      throw Error(
        "LogicHelper not initialized: nonRequiredBossDungeons is null",
      );
    }
  ...
  static #parseRequirement(requirement, isFlattened) {
    ...
    const optionEnabledRequirementValue =
      this.#checkOptionEnabledRequirement(requirement);
    if (!_.isNil(optionEnabledRequirementValue)) {
      return optionEnabledRequirementValue;
    }
    ...
    const predeterminedItemRequirementValue =
      this.#checkPredeterminedItemRequirement(requirement);
    if (!_.isNil(predeterminedItemRequirementValue)) {
      return predeterminedItemRequirementValue;
    }

    const bossRequirementValue = this.#checkBossRequirement(
      requirement,
      isFlattened,
    );
  ...
  static #booleanExpressionForTokens(expressionTokens, isFlattened) {
    ...
    while (!_.isEmpty(expressionTokens)) {
      ...
      if (currentToken === this.TOKENS.AND || currentToken === this.TOKENS.OR) {
        ...
        const parsedRequirement = this.#parseRequirement(
          currentToken,
          isFlattened,
        );
    ...
    if (
      expressionTypeToken === this.TOKENS.AND ||
      itemsForExpression.length <= 1
    ) {
      return BooleanExpression.and(...itemsForExpression);
    }
    // istanbul ignore next
    throw Error(
      `No expression type for items: ${JSON.stringify(itemsForExpression)}`,
    );
  }
