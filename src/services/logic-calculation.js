...
class LogicCalculation {
  ...
  formattedRequirementsForEntrance(entranceName) {
    const requirementsForEntrance =
      LogicHelper.requirementsForEntrance(entranceName);
  ...
  locationsList(generalLocation, { onlyProgressLocations, disableLogic }) {
    ...
    return _.map(detailedLocations, (detailedLocation) => {
      const isAvailable = this.isLocationAvailable(
        generalLocation,
        detailedLocation,
      );
  ...
  itemsRemainingForRequirements(requirements) {
    return requirements.reduce({
      ...
      andReducer: ({ accumulator, item, isReduced }) =>
        accumulator +
        (isReduced ? item : this.itemsRemainingForRequirement(item)),
  ...
  #setGuaranteedKeys() {
    ...
    const isSmallKeyLunacy = Settings.getOptionValue(
      Permalink.OPTIONS.SMALL_KEYLUNACY,
    );
    const isBigKeyLunacy = Settings.getOptionValue(
      Permalink.OPTIONS.BIG_KEYLUNACY,
    );

    if (!isSmallKeyLunacy || !isBigKeyLunacy) {
      _.forEach(LogicHelper.MAIN_DUNGEONS, (dungeonName) => {
        const { guaranteedSmallKeys, guaranteedBigKeys } =
    ...
    return LogicHelper.isLocationAvailableWithSmallKeys(
      ...
      detailedLocation,
      {
        ...
        nonKeyRequirementMet: (requirement) =>
          this.isRequirementMet(requirement),
      },
    );
  }
  ...
  static #parseHasAccessedOtherLocation(requirement) {
    return _.get(
      requirement.match(LogicHelper.HAS_ACCESSED_OTHER_LOCATION_REGEX),
      1,
    );
  }
  ...
  #hasAccessedOtherLocationRequirementRemaining(requirement) {
    const otherLocation =
      LogicCalculation.#parseHasAccessedOtherLocation(requirement);
    if (otherLocation) {
  ...
  #formatRequirements(requirements) {
    ...
    const sortedRequirements = LogicCalculation.#sortRequirements(
      evaluatedRequirements,
    );
    const readableRequirements =
      LogicCalculation.#createReadableRequirements(sortedRequirements);
  ...
  static #createReadableRequirements(requirements) {
    if (requirements.type === this.#BOOLEAN_EXPRESSION_TYPES.AND) {
      return _.map(requirements.items, (item) =>
        _.flattenDeep(
          this.#createReadableRequirementsHelper(item, requirements.value),
        ),
      );
    }
    if (requirements.type === this.#BOOLEAN_EXPRESSION_TYPES.OR) {
      return [
        _.flattenDeep(
          this.#createReadableRequirementsHelper(requirements, false),
        ),
      ];
    }
    // istanbul ignore next
  ...
  static #createReadableRequirementsHelper(requirements, isInconsequential) {
    if (requirements.item) {
      ...
      const otherLocation = this.#parseHasAccessedOtherLocation(
        requirements.item,
      );
    ...
    return _.map(requirements.items, (item, index) => {
      ...
      if (item.items) {
        ...
        currentResult.push([
          ...
          this.#createReadableRequirementsHelper(
            item,
